  ...
  en: {
    ...
    support: {
      ...
      sections: [
        ...
        ["Does the game work offline?", "Yes. Core gameplay works offline. Local score saving is not affected by network availability."],
  ...
  "zh-Hans": {
    ...
    support: {
      ...
      sections: [
        ...
        ["游戏可以离线玩吗？", "可以。核心玩法可离线运行，本地成绩保存不受网络状态影响。"],
  ...
  "zh-Hant": {
    ...
    support: {
      ...
      sections: [
        ...
        ["遊戲可以離線玩嗎？", "可以。核心玩法可離線運作，本機成績儲存不受網路狀態影響。"],
...
const templateSources = {
  fr: {
    ...
    offlineA: "Oui. Le jeu principal fonctionne hors ligne. L'enregistrement local des scores n'est pas affecté par l'état du réseau.",
  ...
  es: {
    ...
    offlineA: "Sí. La jugabilidad principal funciona sin conexión. El guardado local de puntuaciones no depende del estado de la red.",
  ...
  de: {
    ...
    offlineA: "Ja. Das Hauptspiel funktioniert offline. Die lokale Speicherung von Ergebnissen hängt nicht vom Netzwerkstatus ab.",
  ...
  return {
    ...
    support: {
      ...
      sections: [
        ...
        ["Offline play", "Core gameplay works offline. Local score saving is not affected by network availability."],
...
function extraPrivacySections(locale) {
  if (locale.base === "zh-Hans") {
    return [
      ...
      ["离线可玩", "核心玩法不依赖网络连接。本应用没有我们自建的网络服务。"],
      ["Apple 服务", "购买、下载、更新、账单和退款等 App Store 相关流程由 Apple 提供，并按 Apple 的条款和隐私政策处理。我们不会通过这些服务建立自己的用户档案。"],
      ["数据保留与删除", "本地设置和本地成绩会保留在你的设备上，用于维持游戏体验。你可以通过删除应用来删除这些本地数据。"],
      ["数据共享", "我们不会出售、出租或共享你的个人数据。本应用没有我们运营的外部数据接收方。"],
      ...
      ["你的选择", "你可以在应用内关闭音乐、音效和触感反馈，也可以删除应用以移除设备上的本地游戏数据。"],
      ["适用范围", "本政策只适用于《下落》本身，不适用于 App Store、设备系统设置或你通过其他方式访问的第三方网站和服务。"]
    ];
  }
  ...
  if (locale.base === "zh-Hant") {
    return [
      ...
      ["網路連線", "核心玩法不依賴網路連線。本應用程式沒有我們自建的網路介面。"],
      ["Apple 服務", "購買、下載、更新、帳單與退款等 App Store 相關流程由 Apple 提供，並依 Apple 的條款與隱私權政策處理。我們不會透過這些服務建立自己的使用者檔案。"],
      ["資料保留與刪除", "本機設定與本機成績會保留在你的裝置上，用於維持遊戲體驗。你可以透過刪除應用程式來刪除這些本機資料。"],
      ["資料共享", "我們不會出售、出租或共享你的個人資料。本應用程式沒有我們營運的外部資料接收方。"],
      ...
      ["你的選擇", "你可以在應用程式內關閉音樂、音效和觸覺回饋，也可以刪除應用程式以移除裝置上的本機遊戲資料。"],
      ["適用範圍", "本政策只適用於《下落》本身，不適用於 App Store、裝置系統設定或你透過其他方式存取的第三方網站和服務。"]
    ];
  }
  ...
  return [
    ...
    ["Offline Play", "Core gameplay does not depend on an internet connection. The app has no developer-operated network service."],
    ["Apple Services", "App Store purchase, download, update, billing, and refund processes are provided by Apple under Apple's own terms and privacy policy. We do not use those services to build our own user profile about you."],
    ...
    ["Data Sharing", "We do not sell, rent, or share your personal data. The app has no external data recipient operated by us."],
    ...
    ["Your Choices", "You can turn off music, sound effects, and haptics inside the app. You can also delete the app to remove local game data from your device."],
    ["Scope", `This policy applies only to ${locale.appName}. It does not apply to the App Store, device system settings, or third-party websites and services you access outside the app.`]
  ];
}
...
function extraSupportSections(locale) {
  if (locale.base === "zh-Hans") {
    return [
      ["支持范围", "我们可以协助处理应用使用、设置、语言显示、音频触感、成绩保存和明显的崩溃或卡顿问题。"],
      ...
      ["隐私相关问题", "如果你对本地数据或隐私政策有疑问，可以通过支持邮箱联系我们。"],
  ...
  if (locale.base === "zh-Hant") {
    return [
      ["支援範圍", "我們可以協助處理應用程式使用、設定、語言顯示、音訊觸覺回饋、成績儲存，以及明顯的閃退或卡頓問題。"],
      ...
      ["隱私相關問題", "如果你對本機資料或隱私權政策有疑問，可以透過支援信箱聯絡我們。"],
  ...
  return [
    ["Support Scope", "We can help with app usage, settings, language display, audio or haptic behavior, local score saving, and clear crash or performance issues."],
    ...
    ["Privacy Questions", "If you have questions about local data or the privacy policy, contact us by email."],
